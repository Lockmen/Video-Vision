...
high = 255 # graylevel이 반대가 된거 같은데 왜 이게 제대로 작동하는겨???
low = 0

for x in range(1 , row):
    for y in range(1, col):
        if low < gray_img.getpixel((x,y))[0] : # 입력영상 최대값 찾기
            low = gray_img.getpixel((x,y))[0]
        if high > gray_img.getpixel((x,y))[0]: # 입력영상 최소값 찾기
            high = gray_img.getpixel((x,y))[0]

for x in range(1 , row):
    for y in range(1, col):
        if gray_img.getpixel((x, y))[0] >= low:
            stretch_img.putpixel(((x,y)),low)
        elif gray_img.getpixel((x, y))[0] <= high:
            stretch_img.putpixel(((x, y)), high)
        else:
            stretch_img.putpixel((x,y), int((gray_img.getpixel((x,y))[0]-high)*255/(low-high)))
